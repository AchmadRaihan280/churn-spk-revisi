...
export default function DataCustomerPage() {
  ...
  // FILTER UTAMA: Hanya mengambil Churn >= 50% (High & Medium)
  const filteredCustomers = customers
    .filter((item) => {
      const probability = item.final_probability || 0;
      const isHighOrMedium = probability >= 0.5;

      const matchesSearch = item.customerID
        ?.toLowerCase()
        .includes(searchTerm.toLowerCase());

      const itemRisk = item.risk ? item.risk.toLowerCase() : "low";
      const matchesRisk =
        riskFilter === "all" || itemRisk === riskFilter.toLowerCase();

      return isHighOrMedium && matchesSearch && matchesRisk;
    })
    .sort((a, b) => {
  ...
  const handleDownloadCSV = () => {
    ...
    link.setAttribute("download", "Data_Pelanggan_Risiko_Churn.csv");
  ...
  const highRiskCount = filteredCustomers.filter(
    (c) => c.risk?.toLowerCase() === "high",
  ).length;
  const medRiskCount = filteredCustomers.filter(
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-slate-900 flex items-center gap-3">
            ...
            Pelanggan Berisiko Churn (High & Medium)
          </h1>
          <p className="text-slate-500 mt-2 text-lg">
            Daftar pelanggan aktif dengan probabilitas churn di atas 50%.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className={`${cardStyle} border-l-4 border-l-yellow-500`}>
            <p className="text-yellow-600 text-xs font-bold uppercase tracking-wider">
              Medium Risk
            </p>
            <h2 className="text-3xl font-extrabold text-slate-900">
              {medRiskCount}
          ...
          <div className="flex flex-col md:flex-row gap-4 items-center justify-between border-b border-gray-100 pb-6 mb-6">
            <div className="relative w-full md:w-80">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2.5 border border-slate-200 rounded-xl text-sm text-slate-700 bg-slate-50/50"
              />
            </div>
            <div className="flex items-center gap-3 w-full md:w-auto justify-end">
              ...
              <select
                ...
                className="w-full md:w-48 px-3 py-2.5 border border-slate-200 rounded-xl text-sm bg-slate-50 text-slate-700 font-medium"
              >
              ...
              <button
                ...
                className="bg-teal-600 hover:bg-teal-700 text-white font-bold text-sm px-5 py-2.5 rounded-xl shadow-md"
              >
                Export CSV
              </button>
            </div>
              ...
              <tbody className="divide-y divide-gray-100 text-sm">
                {currentItems.length > 0 ? (
                  currentItems.map((customer) => {
                    ...
                    const riskLabelText =
                      customer.risk?.toLowerCase() || "high";

                    // LOGIK PEWARNAAN BERDASARKAN RISK LEVEL
                    let containerStyles =
                    ...
                    let bulletIconColor = "text-green-600";
                    ...
                    return (
                      ...
                      >
                        ...
                        {/* KOLOM REKOMENDASI BERWARNA + SCROLLABLE */}
                        <td className="py-5 px-6">
                          <div
                          ...
                          >
                            ...
                            {!isUploadFile && (
                              <div>
                                ...
                                >
                                  💡 Indikator Risiko ({reasonsList.length})
                                </div>
                            ...
                            <div>
                              ...
                              >
                                🚀 Rekomendasi Tindakan ({actionList.length})
                              </div>
                        ...
                        <td className="py-5 px-6">
                          ...
                          "Sudah Ditindaklanjuti" ? (
                            <div className="flex items-center gap-1.5 text-green-700 font-bold text-xs bg-green-50 border border-green-200 px-3 py-1.5 rounded-xl w-fit">
                              <CheckCircle size={14} /> <span>Selesai</span>
                            </div>
                          ) : (
                            <div className="flex flex-col gap-2">
                              ...
                              <button
                                ...
                                className="bg-teal-600 hover:bg-teal-700 text-white font-bold text-xs px-3 py-2 rounded-xl transition-all shadow-sm"
                              >
                                Tindak Lanjuti
                              </button>
                            </div>
